...
`;
  ...
  if (existsSync(binaryPath)) {
    try {
      ...
      const dataRoot = pathResolve(process.env.LS_DATA_DIR || './data');
      const workspaceDir = '/tmp/windsurf-workspace';
      mkdirSync(`${dataRoot}/db`, { recursive: true });
      mkdirSync(workspaceDir, { recursive: true });
      for (const entry of readdirSync(workspaceDir)) {
        try { rmSync(`${workspaceDir}/${entry}`, { recursive: true, force: true }); } catch {}
      }
    } catch {}
