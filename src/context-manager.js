...
async function callLightweightModel(prompt) {
  try {
    ...
    const timeoutMs = config.contextTrimSummaryTimeoutMs || 15000;
    ...
    if (err.message && err.message.includes('timed out')) {
      console.warn('[context-manager] ALERT: Summary generation timed out, using fallback');
    }
...
export async function processContext(messages, conversationId) {
  ...
  // Fallback: 摘要生成失败时注入兜底文本
  if (!summaryText && trimmedForSummary && trimmedForSummary.length > 0) {
    summaryText = '[Summary unavailable - ' + trimmedForSummary.length + ' earlier messages were compressed]';
  }
